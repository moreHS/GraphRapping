...
### 설계 확장 — Tier 3 역방향 포섭 (2026-07-23, 사용자 설계 지적 반영)

**지적**: "LLM이 성분이라고 추출했는데 사전 키에 없으면 무시된다 — 사전은
성분 리스트가 아니라 명칭 정돈 매핑테이블이어야 한다." 실측 확인: 해석이
정방향(INCI ⊂ 표현)+별칭 키뿐이라 표현이 INCI보다 짧은 관용어('콜라겐' vs
'솔루블콜라겐')는 사전 키 없으면 무조건 낙하 — 사전이 사실상 화이트리스트로
동작.

**구현**: resolve_query_concepts에 **Tier 3(역방향 포섭)** — ①bare ②별칭이
빈손이고 **사전이 커버하지 않는** 표현일 때, 표현⊂INCI인 카탈로그 성분을
채택(콜라겐→2종, 세라마이드→1종, 펩타이드→8종). 가드: 부정 스팬 ·
`_REVERSE_MATCH_CAP=10`(오일 39종 = 과일반 → 기각·미해석 정직 표시) ·
다어절 질의 자기제한(긴 문자열은 어떤 토큰의 부분문자열도 아님). 사전 역할은
원목적으로 축소: 문자열로 못 잇는 관용어(히알루론↔하이알루로네이트) +
큐레이션(알코올=변성알코올만). constraint 빌더를 **표현(matched_text) 단위
그룹**으로 재작성 — 같은 표현의 INCI들 = OR 한 덩어리(콜라겐이 id별 AND
싱글턴 2개로 쪼개지던 잠복 버그 제거), 별칭 패밀리는 name_surfaces 보강 전담.

**codex 포커스 리뷰 REQUEST CHANGES(P1 3·P2 1) → 전부 수정**: ① 큐레이션
우선을 "채택 성공"이 아닌 **사전 커버 여부**로(별칭 타깃이 카탈로그에 없어도
지방알코올 오포섭 차단) ② 다중 표현 질의에서 (type,id) dedupe가 표현→성분
소속을 파괴 — 패밀리 key 표현은 **패밀리 전체 시그니처**로 복원("히알루론
소듐하이알루로네이트" AND 오거부 제거) ③ 익명 search가 원문 빈-해석 시
constraint를 버리고 조기 반환("콜라겐 추천해줘" 0건) — constraint 존재 시
스킵+합성 개념으로 오버랩 부여 ④ cap 스캔 조기 종료 + 45k suffix 인덱스
필요성 주석 기록.

**게이트**: pytest **1555 passed / 50 skipped**(+14), ruff/mypy 클린, 스냅샷
diff 0·byte-identity 그린. 라이브: "콜라겐 추천해줘"(익명·무카테고리) 결과
10/10 함유 · "콜라겐 없는 크림" 기피 2종·누출 0 · 혼합 질의("히알루론
소듐하이알루로네이트") 히알루론 constraint 패밀리 4종 완전체 · 오일 기각 ·
히알루론/알콜업는 회귀 무변.
