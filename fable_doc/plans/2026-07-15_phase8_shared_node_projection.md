...
로드 경로에서 1회 호출**: DBServingStore `_fetch_products` 직후 / DemoServingStore
serving_products 빌드 시 `attach_similarity_signals(products, ...)`. corpus-level
1회(IDF 집계 지점과 일치).
...
- `keyword::{bee_attr_id}:{keyword_id}` — **복합키**(bee_attr 스코핑, 결정4). 신호의
  `keyword_id`+`bee_attr_id` 필드 사용. B2 alias 정규화(canonical keyword) 적용 후 스코핑
- `ingredient::{ingredient_concept_id}`
- `category::{category_concept_id}`
- `brand::{brand_concept_id}`
- `goal::{main_benefit_concept_id}`
- polarity가 있는 축(keyword)은 노드에 극성 포함 or 극성 불일치 시 미카운트
- **bee_attr는 노드 아님** — keyword 스코핑에만 사용

**함수**:
```
build_idf(product_profiles, axes) -> dict[node_key, float]      # 코퍼스 df → IDF=log(N/df)
build_similarity_signals(product_profiles, *, axes, idf,
...
- score(A,B) = Σ_{공유 node} IDF(node) [× polarity_factor]. `category_gate=True`면
  동일 category_group 쌍만(classify_product_category_group)
- top_n 이웃 절단, min_score 하한
- **SimilarProductSignal.to_dict()**: `{product_id, neighbor_name(representative_
  product_name), score, shared_axes:[{axis, node_key, label, idf}]}` — 이웃 라벨·근거
  자체 포함(G2가 코퍼스 접근 없이 렌더 가능하게). **shared_axes 비면 신호 미방출**(계약)

**완료 기준**:
- dense/wide 실측: keyword(bee_attr 복합키)·ingredient·category·brand·goal 각 축
  IDF 가중 후 **기준 상품별 top-N 품질** — (a) category_gate=True 시 top-N 동일
  카테고리 지배 (b) category_gate=False 시 앵커와 공유노드 多 상품이 상위 (c)
  **커버리지**(유사 이웃 ≥1 상품 비율) 보고 (d) bee_attr 미사용·keyword 복합키
  스코핑 확인
- 단위테스트: IDF 계산, 축 네임스페이스 분리, keyword bee_attr 복합키(같은 keyword_id
  다른 bee_attr → 다른 노드 확인), polarity 불일치 미카운트, category_gate on/off,
  shared_axes 강제(비면 미방출)
...
- graph_view.js: **색 1개 추가 외 무수정**(data-driven, 회색 fallback). SHARES_
  ATTRIBUTE는 대칭이라 무방향 스타일 1줄(경미)
- 신규 view 모드 or corpus view 확장(판단)
...
- owned_product_ids 앵커 → 속성-유사 후보 부스트. `BOOST_ONLY_TYPES`에 `similar`
  추가(non-admissible), top-level `similar_product_weight`(features 맵 밖), §13 계약
  5조건, family명 `PRODUCT_SIMILARITY_AFFINITY`
...
**(주의: 이 확정본은 1차 크로스리뷰 이후 설계가 실질 변경됨 — P8-1 착수 전 변경분
재리뷰 권장: 특히 category_gate 파라미터화·keyword 복합키·brand 미배제.)**
