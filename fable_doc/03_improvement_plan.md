...
## Phase 8 — 공유 노드 상품 유사도 그래프 (추가 2026-07-15, 제안)
...
(노드가 차원), ingredient(rarity+게이팅) 주력, ingredient∩main_benefit 최정밀
(노이즈 6.1%), 3홉은 폭발(2홉만). 4.0 audit 재판정: 후보생성 렌즈 MARGINAL 유지·
탐색/유사상품 렌즈 **VIABLE-WITH-GATING** 상향. 상세 계획(정본):
`fable_doc/plans/2026-07-15_phase8_shared_node_projection.md`

- **Track G**: G1 유사도 계산 모듈(Jaccard+IDF+카테고리 게이팅, ephemeral,
  bee_attr 배제) / G2 그래프 뷰어 상품-상품 서브그래프(SHARES_ATTRIBUTE +
  shared_axes tooltip) / G3 유사상품 위젯 API(eligibility 미경유) / G4 re-rank
  boost(boost-only, 조건부) / G5 검색 확장(후순위)
- **원칙**: 그래프DB 불요(self-join), evidence-first(shared_axes 동반), Track D
  계열(D4 — D1/D2와 동일 Jaccard/ephemeral/boost-only 패턴, 단 데이터 준비돼
  실발화 가능한 첫 연결 신호), Track E 도래 시 3홉 자연 확장
