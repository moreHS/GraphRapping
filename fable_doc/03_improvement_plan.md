...
## Phase 8 — 공유 노드 상품 유사도 그래프 (추가 2026-07-15, **확정·재리뷰 완료**)

사용자 질문(2026-07-14): 공유 속성 노드로 상품 2홉/멀티홉 연결이 왜 안 되나.
심층 분석(`fable_doc/07_shared_node_projection_analysis.md`, 리서치 3트랙 실측 +
Relation 프로젝트 패턴 대조) 결론: **연결은 데이터에 실재·조밀(추천엔 암묵 사용
중)하고, 명시적 유사도/탐색 기능만 미구현.** 단 축 선택이 승패 — bee_attr는 함정
(노드가 값 아닌 차원), ingredient 주력, 3홉은 폭발(2홉만). 4.0 audit 재판정:
후보생성 렌즈 MARGINAL 유지·탐색/유사상품 렌즈 **VIABLE-WITH-GATING** 상향.
설계 논의: `DECISIONS/2026-07-15_phase8_shared_node_design_dialogue.md`. 상세
계획(정본): `fable_doc/plans/2026-07-15_phase8_shared_node_projection.md`

- **메커니즘(확정)**: 유사도 = **IDF 가중 공유노드 합 + 랭킹 + top-N**(하드-AND·
  노드병합·전역 게이트 아님). **카테고리 게이팅은 소비 맥락 파라미터**(유사상품=ON /
  일반추천=OFF 다양성 / 쿼리=상류)
- **축(전부 점수 노드)**: keyword(**(bee_attr,keyword,polarity) 복합키**, 주력 —
  "가볍다"가 bee_attr별 다른 의미라 스코핑 필수)·ingredient·category·brand·goal.
  bee_attr 클래스명은 점수 제외·keyword 스코핑 전용. brand/category는 IDF가 쏠림
  자동 보정(하드 배제 안 함)
- **Track G**: G1 계산 모듈(keyword는 raw `wrapped_signal` sidecar 소싱, 나머지 축은
  서빙 프로파일; ephemeral attach) / G2 그래프 서브그래프(SHARES_ATTRIBUTE +
  shared_axes) / G3 유사상품 위젯(eligibility 미경유, gate ON) / G4 일반추천 boost
  (boost-only, gate OFF, 조건부) / G5 검색 확장(후순위)
- **원칙**: 그래프DB 불요(self-join), evidence-first(shared_axes 동반), Track D
  계열(D4), Track E 도래 시 앵커 확장으로 3홉 자연 확장
- **상태**: 사용자 결정 4건 확정 + 3자 재리뷰(Opus/Sonnet/Codex) 반영 완료 —
  keyword 복합키 데이터 소스를 raw sidecar로 확정(집계가 bee_attr 버리는 사실제약
  해소, 기존 파이프라인 무회귀). **P8-1 착수 가능 상태**
- **상태**: 계획 제안 — 크로스리뷰 진행, 사용자 결정 4건(축 조합/1차 활용
  우선순위/materialize/keyword 포함) 대기
