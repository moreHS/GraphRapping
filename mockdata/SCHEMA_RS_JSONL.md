...
│   └── brnd_nm           ← 명시적 source 브랜드명 (추가 예정, 없으면 비움)
├── own 전용 필드
...
| `brnd_nm` | str/null | 명시적 source 브랜드명 (**추가 예정**). 없으면 null/누락으로 두며 `prd_nm` token에서 만들지 않는다. | `"라네즈"` |
| `prd_apal_scr` | number/null | source 리뷰 평점. 없으면 null이며 평균/마스터 평점을 0.0으로 대체하지 않는다. | `5.0` |
...
| `relation` | str | 관계 타입 (68 canonical predicates) |
| `source_type` | str | 추출 소스 (`"NER-NER"`, `"NER-BeE"`) |

**GraphRapping relation-ready 상태**

| 상태 | 조건 | 의미 |
|------|------|------|
| `relation_pending` | `bee_spans[]`는 있으나 `relation[]`가 비어 있음 | relation model 서빙 전 또는 relation 미부착 입력. valid input이지만 BEE를 target-linked signal로 자동 승격하지 않음 |
| `relation_ready` | `relation[]`가 1개 이상 있음 | relation row를 근거로 NER/BEE attribution과 canonical fact 생성을 수행할 수 있는 입력 |

GraphRapping loader는 두 상태를 모두 허용한다. 운영 최종 방향은 relation model이 relation을 붙인 `relation_ready` rs.jsonl을 canonical input으로 사용하는 것이다.
...
| prd_apal_scr | prd_apal_scr | fprh.prd_apal_scr | 리뷰 평점 (없으면 NULL) |
...
| product_id | source_product_id | source product id로 string 보존, ProductIndex exact match 우선 |
| prd_nm | prod_nm | 상품명 기반 매칭 |
| channel | source_channel + clct_site_nm | source channel 보존 및 수집 채널 표시 |
| brnd_nm | brnd_nm | 명시적 source 브랜드명. 없으면 null/missing으로 유지 |
| prd_apal_scr | source_rating | source 리뷰 평점. 없으면 NULL |
