...
### 제약 1: source JSON에 keywords 필드 없음
`hab_rel_sample_ko_withPRD_listkeyword.json`의 `relation[].object`에 `keywords[]` 필드가 **없음**.
Neo4j 파이프라인의 keyword 추출은 이 필드에 의존 (`_process_keywords()`).
→ **keyword 추출을 위해 BEE phrase에서 직접 추출하는 로직 필요** (또는 Neo4j 산출물의 keyword를 직접 사용)
...
현재 `review_ingest.py:162-175`에서 REL row를 만들 때 `obj_start`, `obj_end`는 이미 추가됨.
하지만 `source_type`이 "NER-BeE"일 때 **object의 entity_group이 BEE attr 이름**이라는 정보가 중요.
이미 `obj_group`으로 보존 중 → OK.
...
### 3-3. Keyword 처리 전략 (source에 keywords 없는 문제)

**Option A**: BEE phrase 자체를 keyword로 사용
- BEE phrase normalize → auto keyword_id
- 장점: 구현 간단, 100% coverage
- 단점: keyword가 너무 길거나 noisy

**Option B**: keyword_surface_map.yaml 유지 + auto fallback
- surface map에서 매칭 시도 → 성공하면 사전 keyword 사용
- 실패 시 → phrase normalize하여 auto keyword
- 장점: 사전 품질 유지 + coverage 보장

**Option C**: Neo4j 산출물의 keyword를 seed로 사용
- entities.json에서 KEYWORD type entity 120개 추출
- 이걸 keyword_surface_map.yaml에 추가
- 장점: 이미 검증된 keyword set 활용

→ **권장: Option B + C** (사전을 Neo4j keyword로 확장 + auto fallback)
